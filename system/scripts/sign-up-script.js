...
async function sign_up() 
{
	try 
	{
		...
		let hashed_member_password = sha512(document.getElementById("member_password").value);

		let unformatted_member_dob = new Date(document.getElementById("member_DOB").value);
		let member_dob = unformatted_member_dob.toISOString().split("T")[0];

		let member_gender = document.getElementById("member_gender").value;
		let club_ID = document.getElementById("member_club").value;

		let form_data = new FormData();

		let alert_element = document.getElementById("invalid_input");

		form_data.append("member_fname", member_fname);
		form_data.append("member_lname", member_lname);
		form_data.append("member_email", member_email);
		form_data.append("hashed_member_password", hashed_member_password);
		form_data.append("member_DOB", member_dob);
		form_data.append("member_gender", member_gender);
		form_data.append("club_ID", club_ID);
